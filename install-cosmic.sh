...
xdg-desktop-portal-gtk xdg-user-dirs gvfs gvfs-nfs gvfs-smb nfs-utils cifs-utils \
power-profiles-daemon gnome-keyring libmad rsync cronie git curl wget tar 7zip less \
base-devel bash-completion vim nano micro fzf lazygit htop fastfetch cmus cava ranger \
highlight atool w3m mediainfo perl-image-exiftool kitty python-pillowfight imv mpv \
...
bash ~/arch-cosmic/scripts/install-gpu-packages.sh
...
cp -R ~/arch-cosmic/home/.??* ~/
sudo cp -R ~/arch-cosmic/root/* /
